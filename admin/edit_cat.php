...
<?php 
    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
        try{
            ...
            $update_sql = $connect->prepare("UPDATE categories SET categories.nom_categorie = ? WHERE categories.id_categorie = ?");
            $update_sql -> bind_param("sd", $new_cat_name, $id);
...
<body>
    <div id="container_cat" class="container absolute top-0 left-0 w-screen h-screen bg-black bg-opacity-50">
        <div  class="max-w-md mx-auto mt-8 p-6 bg-white rounded-lg shadow-md absolute top-0 left-[40%]">
            <h2 class="text-2xl font-bold mb-6 text-gray-800">Edit categorie</h2>
            ...
            <form action="<?=htmlspecialchars($_SERVER['PHP_SELF'])?>" method="POST" class="space-y-4">
                ...
                <div>
                    <label for="nom_categorie" class="block text-sm font-medium text-gray-700">nouveau nom de categorie: </label>
                    <input type="text" id="nom_categorie" name="nouveau_nom_categorie" class="p-2 mt-1 block w-full rounded-md border-gray-300 shadow focus:border-blue-500">
                </div>
